...
- **Multi-language parsing** — Tree-sitter symbol & call extraction for Rust, Python, JavaScript, TypeScript, Go, Java, C, C++, C#, Ruby, PHP, Lua, GDScript and Godot shaders (regex fallback for other languages); queries live in `queries/<lang>/*.scm` and can be overridden per project
